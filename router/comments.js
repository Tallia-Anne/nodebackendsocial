...
router.get("/commentsAll/:postesId", async (req, res) => {
    try {
        const postesId = req.params.postesId;

        // Rechercher tous les commentaires associés au postId dans la base de données
        const comments = await db.comments.findAll({
            where: { postesId },
        ...
        res.status(500).json({ error: 'Error fetching comments for post', errorMessage: error.message });
    }
});
